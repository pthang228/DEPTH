...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder_name, encoder_name, num_ents, num_rels, num_static_rels, num_words, h_dim, opn, sequence_len, num_bases=-1, num_basis=-1,
        ...
        # === KIẾN TRÚC HỢP NHẤT: Head B path inline (relation-only, INDUCTIVE) ===
        # Thay nhánh CognTKE precompute bằng 1 path-reasoner gọn, DÙNG CHUNG emb_rel.
        # Khởi nhãn tại subject + lan truyền theo quan hệ trên đồ thị lịch sử gần (NBFNet/RED-GNN style).
        # KHÔNG dùng entity-id embedding -> giữ tính inductive.
        self.use_unified_path = True
        self.path_fuser = None            # không dùng pkl nữa
        self.path_gamma = nn.Parameter(torch.zeros(1))   # init 0 -> sàn = LogCL
        self.cur_split = 'train'
        self.path_dim = 32
        self.path_L = 2
        self.path_rel = nn.Linear(self.h_dim, self.path_dim)       # emb_rel(chung) -> không gian path (message)
        self.path_boundary = nn.Linear(self.h_dim, self.path_dim)  # boundary init từ quan hệ truy vấn
    ...
    def path_head_scores(self, triples, glist):
        ...
        us, vs, ers = [], [], []
        for g in glist:
            us.append(g._src.to(dev)); vs.append(g._dst.to(dev)); ers.append(g.edata['type'].to(dev))
        u = torch.cat(us); v = torch.cat(vs); er = torch.cat(ers)   # [E]
        relw = relw_all[er].unsqueeze(0)                   # [1, E, pd]
        N = self.num_ents; pdim = self.path_dim
        deg = torch.zeros(N, device=dev).index_add_(
            0, v, torch.ones_like(v, dtype=torch.float)).clamp(min=1.0).view(1, N, 1)
    ...
    def _fuse_path(self, scores_ob, triples, t_idx, glist=None):
        ...
        # (đường cũ: pkl precompute) — không dùng trong UnifiedTKG
        if self.path_fuser is None:
            return scores_ob
        pd = self.path_fuser.dense(triples, t_idx, self.cur_split)
        return scores_ob + self.path_gamma * pd
