...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder_name, encoder_name, num_ents, num_rels, num_static_rels, num_words, h_dim, opn, sequence_len, num_bases=-1, num_basis=-1,
        ...
        # === CognTKE path fusion (Pha 2): gamma học, init 0 -> sàn = LogCL ===
        import os as _os
        self.path_gamma = nn.Parameter(torch.zeros(1))
        self.path_fuser = None
        if _os.path.exists('cogntke_path_scores.pkl'):
            from path_fuse import PathFuser
            self.path_fuser = PathFuser('cogntke_path_scores.pkl', num_ents, 304, 30, gpu)
            print("[fusion] loaded CognTKE path scores")
    ...
    def _fuse_path(self, scores_ob, triples, t_idx):
        if self.path_fuser is None:
            return scores_ob
        pd = self.path_fuser.dense(triples, t_idx, self.cur_split)   # [B,N] z-norm, =0 ngoài đồ thị
        # Cơ chế CŨ: gamma vô hướng học được, init 0 -> sàn = LogCL
    ...
    def predict(self,que_pair, sub_graph,T_id, test_graph, num_rels, static_graph, test_triplets, use_cuda):
        with torch.no_grad():
            ...
            if self.pre_type == "all":
                ...
                scores_ob = self._fuse_path(scores_ob, all_triples, T_id)
    ...
    def get_loss(self,que_pair, sub_graph,T_idx, glist, triples, static_graph, use_cuda):
        ...
        if self.pre_type == "all":
            scores_ob, _= self.decoder_ob.forward(embedding, r_emb, all_triples, his_emb,self.pre_weight, self.pre_type)
            self.cur_split = 'train'
            scores_ob = self._fuse_path(scores_ob, all_triples, T_idx)
            score_seq = F.softmax(scores_ob, dim=1)
            score_en = score_seq
